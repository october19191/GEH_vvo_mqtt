...
std::cout<< "Real Power got from LVSST1 is " << sst_1 << std::endl;

sst_1 = device::CDeviceManager::Instance().GetNetValue("LVSST", "AOUT/Ac_Pwr_Rec_Fb");
...
try
{
	...
	if(!sst000.empty())
	{
		std::cout<< "Try to send Qsst to TYPE LVSST1 " << std::endl;
		...
		std::cout<< "Couldn't find device type : LVSST1 " << std::endl;
	}
